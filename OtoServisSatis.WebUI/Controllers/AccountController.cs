    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult UserUpdate(Kullanici kullanici)
        {
            ...
            catch (Exception)
            {
                ...
                ModelState.AddModelError("", "Hataoluştu");
            }
        ...
        public async Task<IActionResult> RegisterAsync(Kullanici kullanici)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var rol = await _serviceRol.GetAsyc(r => r.Adi == "Customer");
                    if (rol == null)
                    {
                    ...
                    await _service.AddAsycn(kullanici);
        ...
        public async Task<IActionResult> LoginAsync(CustomerLoginViewModel customerViewModel)
        {
            try
            {
                var account = await _service.GetAsyc(k => k.Email == customerViewModel.Email && k.Sifre == customerViewModel.Sifre && k.AktifMi ==
                    true);
                if (account == null)
                {

                    ModelState.AddModelError("", "Şifre veya Email Yanlış");
